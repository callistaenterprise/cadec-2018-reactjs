...
export async function getRelatedEntities(keyName, keyValues = [], tableName, attributesToGet = []) {
  ...
  const eq = (i, query, keyName, keyValues) => `${query}${keyName} = :${keyName}${i}${end(keyValues.length, i, ' or ')}`;

  const contains = (i, query, keyName, keyValues) => `${query}contains(${keyName}, :${keyName}${i})${end(keyValues.length, i, ' or ')}`;
  ...
  const buildExpressionAttibuteValues = (keyName, keyValues) => keyValues
    .reduce((e, keyValue, i) => ({...e, [`:${keyName}${i}`]: keyValue}), {});
  const params = {
    TableName: tableName,
    FilterExpression: buildQuery(keyName, keyValues),
    ProjectionExpression: attributesToGet.reduce((pe, attribute, i) =>
...
export async function createEntity(tableName, entity) {
  ...
  return docClient.put(params).promise().then(() => entity);
}
...
export async function deleteEntity(tableName, id, idName = 'id') {
  ...
  return docClient.delete(params).promise().then((err, data) => {
    if (err) {
